...
Jenkins is installed with some basic security provisions. 
Users can log in with a username and password in order to perform operations not available to anonymous users. 
By default, logged-in users have full access to all Jenkins functionality and are managed using the "Jenkins own user database" security realm. 
The default configuration also includes protections against many common threats.
